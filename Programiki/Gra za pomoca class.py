...
class Przeciwnik(Postac):
    def __init__(self,gracz):
        super().__init__()
        self.nazwa = choice(['Goblin','Szkielet','Zombie','Mutant'])
...
class Gracz(Postac):
    def __init__(self):
        ...
        self.zycie = 10
        self.max_zycie = 10
        self.nazwa = input('Wpisz swoja nazwe: ')
...
while gra:
    ...
    if akcja == 'zwiedzaj':
        if randint(0,1) == 0:
            print(f'{gracz1.nazwa} znalazl jaskinie')
        else:
            przeciwnik = Przeciwnik(gracz1)
            print('----------------------------')
            print(f'{gracz1.nazwa} natrafil na {przeciwnik.nazwa}')
            gra = gracz1.walka(przeciwnik)
